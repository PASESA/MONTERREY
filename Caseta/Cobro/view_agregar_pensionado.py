...
class View_agregar_pensionados:
    ...
    def interface(self):
        ...
        etiqueta_reposicion_info = ttk.Label(
            seccion_tarjeta_reposicon, text='¿La tarjeta a registrar es de reposición?: ')
        etiqueta_reposicion_info.grid(row=0, column=0, padx=5, pady=5)

        campo_reposicion = ttk.Combobox(
            seccion_tarjeta_reposicon, width=5, state="readonly", textvariable=self.__variable_es_reposicion)
        ...
        seccion_datos_pensionado = ttk.LabelFrame(
            seccion_superior, text="\t\t\tIngresa los datos del pensionado a registrar")
        ...
        self.campo_numero_tarjeta = ttk.Entry(
            seccion_datos_personales_pensionado, textvariable=self.variable_numero_tarjeta)
        ...
        etiqueta_monto_dato_pension.grid(
            row=0, column=0, padx=5, pady=5, sticky=tk.NW)
        self.campo_monto_dato_pension = ttk.Entry(
        ...
        boton_agregar_pensionado = tk.Button(self.panel_crud,  text='Agregar usuario',
                                             command=self.agregar_pensionado, width=20, font=("Arial", 12), background="red")
        ...
        try:
            ...
            if self.__variable_es_reposicion.get() == "Si":
                respuesta = mb.askyesno(
                    "Advertencia", "¿Estas seguro de que la tarjeta registrada es de reposición? De ser asi no olvides desactivar la antigua tarjeta")
            ...
            if len(resultado) == 0:
                ...
                mb.showinfo(
                    "Información", "El pensionado fue añadido correctamente, realice su pago de pensión para activar la tarjeta")
                self.desconectar()
            else:
                self.variable_numero_tarjeta.set('')
                self.campo_numero_tarjeta.focus()
                mb.showerror(
                    "Error", "Ya existe un pensionado registrado con ese numero de tarjeta")
                return
